 ...
 * @since 3.0
 */
class RObstacle extends RRectangle {

	boolean exclude;
	RVertex topLeft, topRight, bottomLeft, bottomRight, center;
	private RShortestPathRouter router;
	 ...
	 *            the bounds
	 */
	RObstacle(RRectangle rect, RShortestPathRouter router) {
		init(rect);
		this.router = router;
	}
	 ...
	 */
	public boolean containsProper(RPoint p) {
	...
	private void growVertex(RVertex vertex) {
	...
	void init(RRectangle rect) {
		...
		topLeft = new RVertex(x, y, this);
		topLeft.positionOnObstacle = RPositionConstants.NORTH_WEST;
		topRight = new RVertex(x + width - 1, y, this);
		topRight.positionOnObstacle = RPositionConstants.NORTH_EAST;
		bottomLeft = new RVertex(x, y + height - 1, this);
		bottomLeft.positionOnObstacle = RPositionConstants.SOUTH_WEST;
		bottomRight = new RVertex(x + width - 1, y + height - 1, this);
		bottomRight.positionOnObstacle = RPositionConstants.SOUTH_EAST;
		center = new RVertex(getCenter(), this);
	}
	...
	private void shrinkVertex(RVertex vertex) {
		if (vertex.totalCount > 0)
			vertex.shrink();
	}
	...
	/**
	 * @see org.eclipse.draw2d.geometry.RRectangle#toString()
	 */
