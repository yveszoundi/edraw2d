...
class RSegment {
	 ...
	 *            the end vertex
	 */
	RSegment(RVertex start, RVertex end) {
		this.start = start;
		this.end = end;
	}
	 ...
	 */
	double cosine(RSegment otherSegment) {
	 ...
	 */
	long crossProduct(RSegment otherSegment) {
	...
	boolean intersects(int sx, int sy, int tx, int ty) {
		return RGeometry.linesIntersect(start.x, start.y, end.x, end.y, sx, sy,
				tx, ty);
	}
	 ...
	 * @return true if the segments intersect
	 */
	boolean intersects(RPoint s, RPoint t) {
