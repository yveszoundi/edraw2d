 ...
 * the client using {@link RPoint Points}. SWT uses integer arrays when painting
 * polylines and polygons.
 */
public class RPointList implements java.io.Serializable {

	private int[] points = new int[0];
	private RRectangle bounds;
	private int size = 0;
	 ...
	 */
	public void addAll(RPointList source) {
	 ...
	 */
	public RPointList getCopy() {
		RPointList result = new RPointList(size);
	...
	public RPoint getPoint(int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		index *= 2;
		return new RPoint(points[index], points[index + 1]);
	}
	 ...
	 * @since 2.0
	 */
	public RPoint getPoint(RPoint p, int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		index *= 2;
	 ...
	 * @see #setPoint(RPoint, int)
	 * @since 2.0
	 */
	public void insertPoint(RPoint p, int index) {
	 ...
	 * @since 3.1
	 */
	public boolean intersects(RRectangle r) {
		if (r.isEmpty())
			return false;
		...
		for (int i = 0; i < (size - 1) * 2; i += 2) {
			if (RGeometry.linesIntersect(diagonal1x1, diagonal1y1, diagonal1x2,
					diagonal1y2, points[i], points[i + 1], points[i + 2],
					points[i + 3])
					|| RGeometry.linesIntersect(diagonal2x1, diagonal2y1,
	 ...
	 */
	public RPoint removePoint(int index) {
		bounds = null;
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		...
		index *= 2;
		RPoint pt = new RPoint(points[index], points[index + 1]);
		if (index != size * 2 - 2)
	 ...
	 * @since 2.0
	 */
	public void setPoint(RPoint pt, int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
	 ...
	 * @since 2.0
	 */
	public final void translate(RPoint pt) {
		translate(pt.x, pt.y);
	}
	 ...
	 *         PointList
	 * @see RGeometry#polygonContainsPoint(RPointList, int, int)
	 * @since 3.5
	 */
	public boolean polygonContainsPoint(int x, int y) {
		return RGeometry.polygonContainsPoint(this, x, y);
	}
	 ...
	 * @see RGeometry#polylineContainsPoint(RPointList, int, int, int)
	 * @since 3.5
	 */
	public boolean polylineContainsPoint(int x, int y, int tolerance) {
		return RGeometry.polylineContainsPoint(this, x, y, tolerance);
	}
